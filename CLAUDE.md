...
Test target `BangumiTrackerTests` exists (BangumiAPIClientTests, BangumiTrackerSmokeTests, MockURLProtocol), added in the P0-P3 optimization. Schemes are autogenerated — no shared xcscheme is committed. Run tests from the repo root:
...
CI (`.github/workflows/ci.yml`) runs build + test + swiftlint on push.
